...
if (TYPE === 'block' || TYPE === 'component') {
    ...
    NAMES.forEach(NAME => {
        ...
        fs.access(`./src/html/${TYPE}s/${NAME}.html`, (err) => {
            if (err) {
                fs.writeFile(`./src/html/${TYPE}s/${NAME}.html`, `{{#> layout}}{{/layout}}`, (err) => {
                    ...
                    console.log(chalk.green(`HTML File ${NAME}.html is created successfully.`))
                })
            } else {
        ...
        fs.access(`./src/html/_partials/${TYPE}s/${NAME}.html`, (err) => {
            if (err) {
                fs.writeFile(`./src/html/_partials/${TYPE}s/${NAME}.html`, `<!-- ${TYPE.toUpperCase()}: ${NAME} -->`, (err) => {
                    if (err) console.log(chalk.red('Failed'));
                    console.log(chalk.green(`HTML File ${NAME}.html is created successfully.`))
                })
            } else {
        ...
        fs.access(`./src/scss/${TYPE}s/_${NAME}.scss`, (err) => {
            if (err) {
                fs.writeFile(`./src/scss/${TYPE}s/_${NAME}.scss`, `.${NAME} {}`, (err) => {
                    if (err) console.log(chalk.red('Failed'));
                    console.log(chalk.green(`File ${NAME}.scss is created successfully.`))
                })
            } else {
                console.log(chalk.yellow(`File ${NAME}.scss is already exist!`))
            }
